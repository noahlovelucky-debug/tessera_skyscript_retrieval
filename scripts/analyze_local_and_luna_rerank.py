...
def metrics(frame: pd.DataFrame, cutoffs: tuple[int, ...] = (10, 25, 50, 100)) -> dict:
    ...
    for cutoff in cutoffs:
        ...
        values[f"ndcg_at_{cutoff}"] = float((selected * discounts).sum() / max(ideal, 1e-8))
    return values


@torch.inference_mode()
def local_scores(config: dict, candidates: pd.DataFrame, query: str) -> tuple[np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    candidates = candidates[candidates["query"].eq(args.query)].sort_values("rank").reset_index(drop=True)
    if len(candidates) != 100:
        raise ValueError(f"expected exactly 100 candidates, got {len(candidates)}")
    global_scores, local = local_scores(config, candidates, args.query)
    ...
    result = {
        ...
        "methods": {item["method"].iloc[0]: metrics(item) for item in records},
    }
    ...
    for name, value in result["methods"].items():
        lines.append(
            f"| {name} | {value['precision_at_10']:.4f} | {value['ndcg_at_10']:.4f} | "
            f"{value['precision_at_25']:.4f} | {value['ndcg_at_25']:.4f} | "
            f"{value['precision_at_100']:.4f} | {value['ndcg_at_100']:.4f} |"
        )
    lines += [
        "",
